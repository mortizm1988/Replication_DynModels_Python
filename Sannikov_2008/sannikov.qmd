...
def F_ode(W: float, Y: np.array) -> np.array:
    ...
    c_star = 0 if Fp >= 0 else minimize(lambda c: c + Fp*u(c), 0.5, bounds=[(0.0, 2.0)]).x[0]
...
root = fsolve(lambda Fp: shooting(Fp),0.5)
...
for W, F, Fp in zip(W_vals, F_W, Fp_W):
    c = 0 if Fp >= 0 else minimize(lambda c: c + Fp*u(c), 0.5, bounds=[(0.0, 2.0)]).x[0]
    a_solution = minimize(
