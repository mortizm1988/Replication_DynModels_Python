...
# -------------------------------------------------
def c_optimal_func(Fp: float) -> float:
    """
    c_opt(W) = (-F'(W)/2)**2.
    """
    if Fp >= 0:   # probationary interval
        return 0
    return ((-Fp)/2)**2
...
def F_ode(W: float, Y: NDArray[np.float64]) -> NDArray[np.float64]:
    ...
    #c_star = c_optimal_func(Fp)    
    c_solution = minimize(lambda c: c+Fp*u(c), 0.5, bounds=[(0.0, 2.0)])
    c_star = c_solution.x[0]
    a_solution = minimize(lambda a: - (gamma(a)*a-h(a)), 0.5, bounds=[(0.0, 1)])
    a_star = a_solution.x[0]

    gamma_star = gamma(a_star)
    drift = W - u(c_star) + h(a_star)
    F_pp = (F - a_star + c_star - Fp*drift) / (0.5*r*(sigma**2)*(gamma_star**2))
    return [Fp, F_pp]
...
def shooting(Fp_guess,return_sol=False):
    ...
    Second: for each Fprime(0),  find W_gp, by finding the first case where  F'(W_gp) = -2*W_gp.
    
    Third: find the Fprime(0) such that its W_gp associated meet the condition F(W_gp) = -W_gp^2
    """
    Fp_guess = float(Fp_guess)
    w_grid = np.linspace(0, 1.2, 100)  
    # First:     
...
# root finding for Fp
root = fsolve(lambda Fp: shooting(Fp),40)
...
for i in range(len(W_vals)):
    ...
    c_solution = minimize(lambda c: c+Fp*u(c), 0.5, bounds=[(0.0, 2.0)])
    c=c_solution.x[0]
    a_solution = minimize(lambda a: (F - a + c - Fp * (W - u(c) + h(a))) / (r * gamma(a)**2 * sigma**2 / 2), 0.5, bounds=[(0.0, 1)])
    a_vals.append(a_solution.x[0])
    c_vals.append(c)
